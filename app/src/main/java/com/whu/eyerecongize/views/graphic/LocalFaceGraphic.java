...
import com.whu.eyerecongize.bilnk.BlinkCheck;
import com.whu.eyerecongize.util.CommonUtils;
...
public class LocalFaceGraphic extends BaseGraphic {
    ...
    public void draw(Canvas canvas) {
        ...
        paintKeyEyes(this.faces.get(0), canvas);
...
//        }
     ...
     * @param canvas canvas
     */
    private void paintFaceShape(MLFace face, Canvas canvas) {//画855个关键点
        if (face == null) {
            return;
        }
    ...
    private void paintKeyPoint(MLFace face, Canvas canvas) {
        for (MLFaceShape contour : face.getFaceShapeList()) {//获取14种轮廓，每个轮廓一组点
            if (contour == null) {
                continue;
            }
        ...
        for (MLFaceKeyPoint keypoint : face.getFaceKeyPoints()) {//获取14个关键点（中心点）
            if (keypoint != null) {
    ...
    private void paintKeyEyes(MLFace face, Canvas canvas) {
        ...
        if(leftEye!=null&&rightEye!=null){
            ...
            for (int i = 0; i < rightpoints.size(); i++) {
                MLPosition point = rightpoints.get(i);
                if (point == null) {
                    continue;
                }
                //canvas.drawPoint(this.translateX(point.getX().floatValue()), this.translateY(point.getY().floatValue()), this.faceFeaturePaint);
